  ...
  const applyLayout = useCallback(
    animate => {
      ...
      panels.forEach((panel, i) => {
        ...
        tl.to(panel, { flexGrow: isActive ? grow : 1, '--ag-dim': isActive ? 0 : 0.2, ...rotProp, duration: dur, ease }, 0);
  ...
  return (
    <div
      ...
      style={{
        ...
        '--ag-gap': `${gap}px`,
        '--ag-radius': `${radius}px`,
        height: vertical ? `680px` : `${height}px`
      }}
      role="group"
      aria-label="AI 图片作品折叠画廊"
    >
      {items.map((item, i) => {
        const isActive = i === active;
        const Tag = 'button';
          ...
          >
            <span className="ag-panel__frame">
              <span className="ag-panel__media" ref={el => (mediaRefs.current[i] = el)}>
                <img loading="lazy" src={item.image} alt={item.alt || item.label || ''} draggable="false" />
              </span>
            ...
            {showLabels && (
              <span className="ag-panel__label" aria-hidden="true">
                ...
                <span className="ag-panel__text" ref={el => (textRefs.current[i] = el)}>
                  {item.label}<small>{item.type} · 点击查看完整作品 ↗</small>
                </span>
              </span>
            )}
          </Tag>
